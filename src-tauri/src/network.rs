...
fn ping_host(host: &str, timeout_ms: u32) -> Result<ConnectivityResult> {
    ...
    if stdout_lower.contains("ttl=") {
        // Extract latency: "time=12ms" or "time<1ms"
        let latency = stdout
            .lines()
            .find(|l| l.to_lowercase().contains("time="))
            .and_then(|line| {
                let time_part = line.split("time=").nth(1)?;
                let ms_str: String = time_part.chars().take_while(|c| c.is_ascii_digit()).collect();
                ms_str.parse::<u64>().ok()
            });
